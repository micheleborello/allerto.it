...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($locked) {
        ...
        $slug     = trim((string)($_POST['slug'] ?? ($prefSlug ?: '')));
...
<body>
        ...
        <form method="post" novalidate>
          ...
          <div class="mb-3">
            ...
            <div class="btn-group" role="group" aria-label="Tipo di accesso">
              ...
              <input type="radio" class="btn-check" name="tipo" id="tipoTenant" value="tenant" autocomplete="off" required <?= ($POST_tipo ?? '')==='tenant' || ($POST_tipo ?? '')==='' ?'checked':'' ?>>
  ...
  <script>
    ...
    function toggleSlug(){
      if (slugWrap) slugWrap.style.display = (tipoTenant && tipoTenant.checked) ? '' : 'none';
      updateForgotLink();
    }
