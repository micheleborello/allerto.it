...
// ---- SHIM caserme (se non già presenti) ----
...
// Pre-calcola UNA SOLA VOLTA l'hash della password iniziale (più veloce nei loop)
...
function _patenti_from_special(array $specializzazioni): array {
  ...
  foreach (($specializzazioni ?? []) as $sp) {
    ...
    if (preg_match('/AUTISTA DI\s+([123])\^/u', $desc, $m)) $out[] = $m[1].'° grado';
  }
...
function _norm_ascii(string $s): string {
  ...
  if (class_exists('Normalizer')) {
    ...
    $s = preg_replace('/\p{Mn}+/u', '', $s); // segni diacritici
    $s = Normalizer::normalize($s, Normalizer::FORM_C);
  } elseif (function_exists('iconv')) {
    $tmp = @iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $s);
    if ($tmp !== false) $s = $tmp;
  }
  // 4) apostrofi e simili -> punto, poi solo [a-z0-9.]
  $s = str_replace(['’','‘','ʼ','´','`',"'"], '.', $s);
  $s = preg_replace('/[^a-z0-9.]+/', '.', $s);
  // 5) compatta punti e togli bordo
  $s = preg_replace('/\.{2,}/', '.', $s);
  $s = trim($s, '.');
  return $s;
}
...
function _username_alt_forms(array $v): array {
  // restituisce [cognome.nome, nome.cognome] (normalizzati) se disponibili
  $cogn = _norm_ascii((string)($v['cognome'] ?? ''));
  $nome = _norm_ascii((string)($v['nome'] ?? ''));
  $out = [];
  if ($cogn !== '' || $nome !== '') {
    $out[] = ($cogn !== '' ? $cogn : 'user').'.'.($nome !== '' ? $nome : 'x');
    $out[] = ($nome !== '' ? $nome : 'x').'.'.($cogn !== '' ? $cogn : 'user');
  }
  // dedup
  $seen=[]; $res=[];
  foreach ($out as $u) { $k = strtolower($u); if (!isset($seen[$k])) { $seen[$k]=1; $res[]=$u; } }
  return $res;
}
...
foreach ($bySede as $codSede => $rows) {
  ...
  foreach ($vigili as &$vv) {
    ...
    // Non ha username: prova a collegare a uno esistente con le forme alternative (cognome.nome / nome.cognome)
    $alts = _username_alt_forms($vv); // già normalizzati
    $matched = false;
    foreach ($alts as $a) {
      $la = strtolower($a);
      if (isset($usersByUsername[$la])) {
        $vv['username'] = $a;
        $matched = true;
        $changedVigili = true;
        break;
    ...
    // Nessun match: genera base stabile (cognome.nome) e rendilo univoco
...
foreach ($casermeAttuali as $slug=>$name) {
  if (is_dir($DATA_BASE.'/'.$slug)) $casermeRows[] = ['slug'=>$slug, 'name'=>$name];
}
